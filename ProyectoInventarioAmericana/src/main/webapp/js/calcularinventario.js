...
$(document).ready(function() {
	...
	$.getJSON(server + 'GetParametro?parametro=REPINMPOSBODEGA' , function(data3){
        replicaInmPOSBodega = data3.valortexto;
    });
			...
			'buttons'	: {
				'Si'	: {
					...
					'action': function(){
							...
							$.ajax({ 
							    		url: server + 'InsertarDespachoTienda?fechasurtir=' + fechasurtir + "&idtienda=" + idtienda + "&observacion=" + observacion + "&estado=" + estado, 
							    		dataType: 'json', 
							    		async: false, 
							    		success: function(data1){ 
							    			respuesta = data1[0];
							    			...
							    			}else
							    			{
							    				...
							    				}else
							    				{
					    							...
					    							//Previo se hace validación de si se debe o no replicar
					    							if(replicaInmPOSBodega == 'S')
					    							{
