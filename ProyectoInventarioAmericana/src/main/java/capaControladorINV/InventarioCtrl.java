...
public class InventarioCtrl {
	...
	{
		//Se debe validar la situaci�n que las llaves est�n disponibles
...
public String editarInsumo(Insumo insumo)
{
	...
	//En este punto realizamos el cambio para realizar la actualizaci�n de precios en lo que va de la semana
...
public void GenerarDespachoTiendaFormatoExcel(int idtienda, String fecha, int idDespacho)
{
	...
	try
	{
		...
		correo.setUsuarioCorreo(infoCorreo.getCuentaCorreo());
		System.out.println("PROBANDO VERSI�N OJOOO " + infoCorreo.getCuentaCorreo() + " " + infoCorreo.getClaveCorreo());
...
public String consultarConsumos(int idTienda, String fechaInicial, String fechaFinal, String tipoConsulta)
{
	...
	/**
	 * M�todo para la replica del retiro de inventario en la tienda Bodega, m�todo que se ejecuta como servicio con el fin de
	 * tenerlo para confirmaci�n del retiro de inventario en BODEGA
	 * @param idDespacho
	 */
	public String insertarRetiroInventarioBodega(int idDespacho)
	{
		...
		//Variable donde almacenamos el resultado de la inserci�n del encabezado
		boolean insercionEnc = false;
		//Vamos llevando un control de la inserci�n de cada detalle
		boolean insercionDet = false;
		//Variable para controlar si hubo error en la inserci�n del despacho en la tienda
		...
		//REALIZAREMOS LA INSERCI�N DEL DESPACHO EN C
		...
		if(!verificarExisteDespacho)
		{
			...
			//Verificaremos si hay m�s de un detalle para insertar
			
			if(detallesDespacho.size() > 0)
			{
				//En este punto hacemos la inserci�n del encabezado
			...
			for(InsumoDespachoTiendaDetalle detalleDespacho: detallesDespacho)
			{
				...
				if(insercionEnc)
				{
					//Realizamos la homologaci�n del idInsumo entre bodega y tienda
					...
					//Es porque hubo error en la inserci�n de un detalle, por lo cual devolvemos y notificamos el error
					if(!insercionDet)
					{
						...
						correo.setContrasena(infoCorreo.getClaveCorreo());
						correo.setUsuarioCorreo(infoCorreo.getCuentaCorreo());
						correo.setMensaje("A continuaci�n informamos que la tienda " + insumoDespacho.getTienda() + " tuvo problemas en la creaci�n del detalle del despacho de pedido. ");
				...
				}else
				{
					...
					correo.setContrasena(infoCorreo.getClaveCorreo());
					correo.setUsuarioCorreo(infoCorreo.getCuentaCorreo());
					correo.setMensaje("A continuaci�n informamos que la tienda " + insumoDespacho.getTienda() + " tuvo problemas en la creaci�n del encabezado del despacho de pedido. ");
					ControladorEnvioCorreo contro = new ControladorEnvioCorreo(correo, correosError);
					contro.enviarCorreoHTML();
					break;
				}
			}
			//En caso de que no hayamos tenido problemas en la inserci�n del encabezado es posible el env�o
			//del correo
			if(insercionEnc)
			{
				//Validamos si no hubo error en la inserci�n del despacho y si no es as�, enviamos correos con la confirmaci�n
				Correo correo = new Correo();
				correo.setAsunto("REPLICA DE DESPACHO EN BODEGA - PARA LA TIENDA " + insumoDespacho.getTienda() + " " + insumoDespacho.getFechaDespacho());
				CorreoElectronico infoCorreo = ControladorEnvioCorreo.recuperarCorreo("CUENTACORREOREPORTES", "CLAVECORREOREPORTE");
				correo.setContrasena(infoCorreo.getClaveCorreo());
				correo.setUsuarioCorreo(infoCorreo.getCuentaCorreo());
				correo.setMensaje("Se ha replicado correctamente el retiro en la BODEGA para la tienda " + insumoDespacho.getTienda()  + " el despacho de Inventario n�mero " + idDespacho);
				ControladorEnvioCorreo contro = new ControladorEnvioCorreo(correo, correosExitoso);
				contro.enviarCorreoHTML();
			}
		}else
		{
			//Enviamos correo indicando que se est� intentando ingresar un despacho que ya existe
			Correo correo = new Correo();
			correo.setAsunto("ERROR RETIRO REPETIDO " + insumoDespacho.getFechaDespacho() + " , despacho " + idDespacho );
			CorreoElectronico infoCorreo = ControladorEnvioCorreo.recuperarCorreo("CUENTACORREOREPORTES", "CLAVECORREOREPORTE");
			correo.setContrasena(infoCorreo.getClaveCorreo());
			correo.setUsuarioCorreo(infoCorreo.getCuentaCorreo());
			correo.setMensaje("A continuaci�n informamos que la tienda " + insumoDespacho.getTienda() + " tuvo problemas se est� intentando ingresar un posible despacho que ya existe. El despacho es el  n�mero " + insumoDespacho.getIdDespacho());
			ControladorEnvioCorreo contro = new ControladorEnvioCorreo(correo, correosError);
			contro.enviarCorreoHTML();
		}
		JSONObject respuesta = new JSONObject();
