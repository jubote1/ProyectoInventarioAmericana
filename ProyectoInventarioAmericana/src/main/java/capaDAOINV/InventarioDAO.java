...
public class InventarioDAO {
	...
	public static ArrayList<InsumoTienda> ObtenerInsumosTiendaAutomatico(int idtien, String fecha)
	{
		...
		{
			Statement stm = con1.createStatement();
			String consulta = "select a.idinsumo, a.cantidad, b.nombre_insumo, b.control_cantidad, IFNULL(c.color_categoria,'') as color_categoria  from insumo_tienda a, insumo b left outer join categoria_despacho_insumo c on b.idcategoria_despacho = c.idcategoria_despacho where a.idinsumo = b.idinsumo and idtienda = " + idtien + " and fecha = '" + fecha + "'";
			logger.info(consulta);
			ResultSet rs = stm.executeQuery(consulta);
			int idinsumo;
			double cantidad;
